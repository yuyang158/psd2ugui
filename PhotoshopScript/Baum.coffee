class Baum
  ...
  runOneFile: (after_close) =>
    @saveFolder = Folder.selectDialog("选择目标文件夹") if @saveFolder == null
    ...
    @removeCommentoutLayers(copiedDoc, copiedDoc)
...
class Util
  ...
  @getTextExtents: (text_item) ->
    ...
    return { x: Math.round(text_item.position[0]), y: Math.round(text_item.position[1]),
    width: Math.round(width * x_scale), height: Math.round(height * y_scale) }
